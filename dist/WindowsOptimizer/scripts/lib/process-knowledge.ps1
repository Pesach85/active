# Process knowledge enrichment â€” deterministic first, cache, KB, web, optional LLM.
        ...
        Get-ChildItem -LiteralPath $dir -Filter '*.md' -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            try {
                ...
                if ($line) {
                    ...
                    if ($snippet.Length -gt 120) { $snippet = $snippet.Substring(0, 120) + 'â€¦' }
...
function Build-ProcessKnowledgeHint {
    ...
    if ($Catalog) {
        ...
        if (Get-Command Resolve-ProcessNecessity -ErrorAction SilentlyContinue) {
            ...
            if ($nec.Level -ne 'Unknown') {
                ...
                $businessHint = 'Already classified in process-intelligence.json â€” no merge needed.'
            }
        }
    }
    ...
    if (-not $whatItIs) { $whatItIs = "Windows process '$name' â€” insufficient local facts" }
    ...
    if (@($suggestedActions).Count -eq 0) {
        [void]$suggestedActions.Add('Identify owner application in Task Manager â†’ Properties â†’ path')
